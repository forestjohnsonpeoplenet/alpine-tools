...
ENV CONFD_VERSION=v0.11.1 \
    CONFD_FORK=forestjohnsonpeoplenet \
...
RUN mkdir -p /opt/src; cd /opt/src && \
    mkdir -p ${SERVICE_VOLUME}/monit/conf.d ${SERVICE_VOLUME}/scripts ${CONFD_HOME}/etc/templates ${CONFD_HOME}/etc/conf.d ${CONFD_HOME}/bin /log && \
    git clone -b "$CONFD_VERSION" "https://github.com/$CONFD_FORK/confd.git" && \
    cd "$GOPATH/confd/src/github.com/$CONFD_FORK/confd" && \
    GOPATH=$GOPATH/confd/vendor:$GOPATH/confd go build && \
    mv ./confd ${CONFD_HOME}/bin/ &&     chmod +x ${CONFD_HOME}/bin/confd && \
    curl -L ${JQ_URL}/jq-${JQ_VERSION}/jq-linux64 -o ${SERVICE_VOLUME}/scripts/jq && \
    chmod 755 ${SERVICE_VOLUME}/scripts/jq &&     cd ${SERVICE_VOLUME} && \
    tar czvf ${SERVICE_ARCHIVE} * &&     apk del go git gcc musl-dev make libressl-dev && \
